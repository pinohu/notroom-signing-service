...
const logStep = (step: string, details?: any) => {
  ...
  console.log(`[TC-CHECKOUT] ${step}${detailsStr}`);
};
...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment", // Transaction coordination is a one-time payment per transaction
      success_url: `${origin}/transaction-coordination/application/success?session_id={CHECKOUT_SESSION_ID}&application_id=${applicationId}`,
      cancel_url: `${origin}/transaction-coordination/application?step=3&application_id=${applicationId}`,
      metadata: {
    ...
    logStep("ERROR in tc-checkout", { message: errorMessage });
